...
function StateDetailPanel({ row, flags }: { row: StateRiskRow; flags: BBQSFlags }) {
...
export default function StatePrivacyMap() {
  const matrix = useMemo(() => getFullMatrix(), []);
      ...
      <div className="px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground max-w-2xl">
            ...
            Toggle your dataset's data types below, then click a state to see specific rules.
          </p>
        </div>
          ...
          <div>
            <Card className="border-border sticky top-20">
              <CardContent className="p-5">
                {selectedRow ? (
                  <StateDetailPanel row={selectedRow} flags={flags} />
                ) : (
